...
softmax_layer make_softmax_layer(int batch, int inputs, int groups)
{
    ...
    l.gpu_load = gpu_load_softmax_layer_output;
    l.gpu_unload = gpu_unload_softmax_layer_output;
...
void pull_softmax_layer_output(const softmax_layer l)
{
    ...
    cuda_pull_array(l.loss_gpu, l.loss, l.inputs*l.batch);
    cuda_pull_array(l.delta_gpu, l.delta, l.inputs*l.batch);
}

void gpu_unload_softmax_layer_output(softmax_layer l, network net)
{
...
void push_softmax_layer_output(const softmax_layer l)
{
    cuda_push_array(l.output_gpu, l.output, l.inputs*l.batch);
    cuda_push_array(l.loss_gpu, l.loss, l.inputs*l.batch);
    cuda_push_array(l.delta_gpu, l.delta, l.inputs*l.batch);
}

void gpu_load_softmax_layer_output(softmax_layer l, network net) {
    l.output_gpu = cuda_make_array(l.output, l.inputs*l.batch); 
    l.loss_gpu = cuda_make_array(l.loss, l.inputs*l.batch); 
